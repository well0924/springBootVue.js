...
export default {
  ...
  methods: {
    ...
    fnSave() {
      ...
      if (this.idx === undefined) {
        //INSERT
        this.$axios.post(apiUrl, this.form,{ headers: {Authorization : localStorage.getItem('user_token')}})
            .then((res) => {
              alert('글이 저장되었습니다.')
        ...
        //UPDATE
        this.$axios.put('//localhost:8090/api/board/'+this.idx, this.form,{ headers: {Authorization : localStorage.getItem('user_token')}})
            .then((res) => {
              alert('글이 저장되었습니다.')
              this.fnView(res.data.id)
            }).catch((err) => {
